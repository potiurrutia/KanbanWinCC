...
export function CreateTeamModal({ open, onClose, onCreate }) {
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <div className="p-6">
        <h2 className="text-lg font-semibold text-slate-900">Nuevo equipo</h2>
        <p className="mt-0.5 text-sm text-slate-500">Crea un espacio para organizar proyectos.</p>
        <form onSubmit={submit} className="mt-5">
          <label className="mb-1 block text-sm font-medium text-slate-700">Nombre del equipo</label>
          <input
            autoFocus
            value={name}
...
export function JoinTeamModal({ open, onClose, onJoin }) {
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <div className="p-6">
        <h2 className="text-lg font-semibold text-slate-900">Unirse a un equipo</h2>
        <p className="mt-0.5 text-sm text-slate-500">Introduce el código de invitación que te han compartido.</p>
        <form onSubmit={submit} className="mt-5">
          <label className="mb-1 block text-sm font-medium text-slate-700">Código de invitación</label>
          <input
            autoFocus
            value={code}
            onChange={(e) => setCode(e.target.value.toUpperCase())}
            placeholder="EJ: A1B2C3D4"
            className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 font-mono text-sm uppercase tracking-widest outline-none transition focus:border-indigo-400 focus:bg-white focus:ring-4 focus:ring-indigo-100"
          />
          {error && (
